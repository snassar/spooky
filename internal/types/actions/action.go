...
// Action represents an action to be acted on machines
type Action struct {
	// Basic action information
	Name        string `hcl:"name,label" validate:"required"`
	Description string `hcl:"description,optional"`
	Type        string `hcl:"type,optional" validate:"omitempty,oneof=command script template_deploy template_evaluate template_validate template_cleanup file_copy"`

	// Execution configuration
	Command   string            `hcl:"command,optional"`
	Script    string            `hcl:"script,optional"`
	Template  *TemplateConfig   `hcl:"template,block"`
	FileCopy  *FileCopyConfig   `hcl:"file_copy,block"`
	Variables map[string]string `hcl:"variables,optional" validate:"omitempty,dive,keys,required,endkeys,required"`
...
type ActionCollection struct {
	...
	Dependencies []*CollectionDependency `hcl:"dependencies,block"`
	Priority     int                     `hcl:"priority,optional" validate:"omitempty,min=1,max=10"`
	Parallel     bool                    `hcl:"parallel,optional"`
	Timeout      int                     `hcl:"timeout,optional" validate:"omitempty,min=1,max=3600"`
...
// FileCopyConfig represents file copy configuration for actions
...
func NewAction(configAction *spookytypesconfig.Action) *Action {
	if configAction == nil {
		return nil
	}
	...
	action := &Action{
		...
		Variables:         configAction.Variables,
	...
	// Convert file copy config if present
	if configAction.FileCopy != nil {
		action.FileCopy = &FileCopyConfig{
			Source:      configAction.FileCopy.Source,
			Destination: configAction.FileCopy.Destination,
			Backup:      configAction.FileCopy.Backup,
			Permissions: configAction.FileCopy.Permissions,
			Owner:       configAction.FileCopy.Owner,
			Group:       configAction.FileCopy.Group,
		}
	}
...
func (a *Action) ToConfigAction() *spookytypesconfig.Action {
	if a == nil {
		return nil
	}

	configAction := &spookytypesconfig.Action{
		Name:              a.Name,
	...
	// Convert template config if present
	if a.Template != nil {
		configAction.Template = &spookytypesconfig.TemplateConfig{
	...
	if a.ResourceLimits != nil {
		configAction.ResourceLimits = &spookytypesconfig.ActionResourceLimits{
