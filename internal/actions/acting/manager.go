...
type Manager struct {
	...
	sessions map[string]*spookyactionstypes.ActingSession
...
func NewManager(
	...
	return &Manager{
		...
		sessions:          make(map[string]*spookyactionstypes.ActingSession),
		logger:            logger,
	}
}
...
func (m *Manager) ExecuteAction(ctx context.Context, action *spookyactionstypes.Action, context *spookyactionstypes.ActionContext) (*spookyactionstypes.ActingSession, error) {
	...
	if err := m.resultProcessor.ProcessResult(result); err != nil {
		m.logger.Warn("Failed to process result", spookylogging.Error(err))
	}
	...
	if err := m.sessionManager.CompleteSession(session.SessionID); err != nil {
		m.logger.Warn("Failed to complete session", spookylogging.Error(err))
	}

	// Update session
	if err := m.sessionManager.UpdateSession(session); err != nil {
		m.logger.Warn("Failed to update session", spookylogging.Error(err))
	}

	m.logger.Info("Action execution completed",
		spookylogging.String("action", action.Name),
		spookylogging.String("session", session.SessionID))
...
func (m *Manager) ExecuteActionCollection(ctx context.Context, collection *spookyactionstypes.ActionCollection, context *spookyactionstypes.ActionContext) (*spookyactionstypes.ActingSession, error) {
	...
	if err := m.progressTracker.StartTracking(session.SessionID, len(collection.Actions)); err != nil {
		m.logger.Warn("Failed to start progress tracking", spookylogging.Error(err))
	}

	// Execute actions
	var results []*spookyactionstypes.ActingResult
	var wg sync.WaitGroup
	resultChan := make(chan *spookyactionstypes.ActingResult, len(collection.Actions))
	...
	if m.defaultParallel && len(collection.Actions) > 1 {
		...
		for _, action := range collection.Actions {
			wg.Add(1)
			go func(action *spookyactionstypes.Action) {
				defer wg.Done()
				semaphore <- struct{}{}
		...
		for i, action := range collection.Actions {
			...
			if err := m.progressTracker.UpdateProgress(session.SessionID, i+1); err != nil {
				m.logger.Warn("Failed to update progress", spookylogging.Error(err))
			}
		}
	}
	...
	if err := m.resultProcessor.ProcessResults(results); err != nil {
		m.logger.Warn("Failed to process results", spookylogging.Error(err))
	}

	// Aggregate results
	aggregatedSession, err := m.resultProcessor.AggregateResults(results)
	if err != nil {
		m.logger.Warn("Failed to aggregate results", spookylogging.Error(err))
	} else {
	...
	if err := m.progressTracker.CompleteTracking(session.SessionID); err != nil {
		m.logger.Warn("Failed to complete progress tracking", spookylogging.Error(err))
	}

	m.logger.Info("Action collection execution completed",
		spookylogging.String("session", session.SessionID),
		spookylogging.Int("total_actions", len(collection.Actions)),
		spookylogging.Int("successful_actions", len(results)))
...
func (m *Manager) PrepareAction(action *spookyactionstypes.Action, context *spookyactionstypes.ActionContext) error {
	...
	m.logger.Info("Action prepared", spookylogging.String("action", action.Name))
	return nil
}

// CreateActor creates an actor for an action
func (m *Manager) CreateActor(action *spookyactionstypes.Action, context *spookyactionstypes.ActionContext) (Actor, error) {
	if action == nil {
		return nil, fmt.Errorf("action cannot be nil")
	}
	...
	actor := &actorImpl{
		action:   action,
		context:  context,
		state:    spookyactionstypes.ActingStatePending,
		status:   spookyactionstypes.ActingStatusPending,
	...
	m.logger.Debug("Created actor", spookylogging.String("action", action.Name))
	return actor, nil
}

// GetActor gets an actor for an action
func (m *Manager) GetActor(action *spookyactionstypes.Action) (Actor, error) {
	if action == nil {
		return nil, fmt.Errorf("action cannot be nil")
	}
...
func (m *Manager) GetSession(sessionID string) (*spookyactionstypes.ActingSession, error) {
	return m.sessionManager.GetSession(sessionID)
}

// ListSessions lists all sessions
func (m *Manager) ListSessions() ([]*spookyactionstypes.ActingSession, error) {
...
func (m *Manager) executeActionWithRetry(ctx context.Context, action *spookyactionstypes.Action, context *spookyactionstypes.ActionContext) (*spookyactionstypes.ActingResult, error) {
	var lastErr error
	...
	for attempt := 0; attempt <= m.retryAttempts; attempt++ {
		if attempt > 0 {
			m.logger.Info("Retrying action execution",
				spookylogging.String("action", action.Name),
				spookylogging.Int("attempt", attempt+1),
				spookylogging.Int("max_attempts", m.retryAttempts+1))
